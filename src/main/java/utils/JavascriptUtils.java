...
public class JavascriptUtils {
	...
	/**
	 * Inject JQuery if not present and wait up to the specified seconds for JQuery
	 * to load.
	 *
	 * @param driver
	 *            the {@link WebDriver} instance
	 * @param seconds
	 *            the explicit number of seconds to wait for jquery to load
	 */
	public static JavascriptExecutor injectJQuery(WebDriver driver, int seconds) {
		...
		if (!isJQueryLoaded(driver)) {
			...
			jsexec.executeScript("var headID = document.getElementsByTagName('head')[0];"
					...
					+ "newScript.src = '" + JQUERY_URL + "';" + "headID.appendChild(newScript);");
	 ...
	 *
	 * @param element
	 *            the {@link WebElement} object;
	 * @return the absolute xpath
	 */
	public static String getAbsoluteXPath(WebElement element) {
		WebDriver driver = ((WrapsDriver) element).getWrappedDriver();
		return (String) executor(driver).executeScript("function absoluteXPath(element) {" + "var comp, comps = [];"
				+ "var parent = null;" + "var xpath = '';" + "var getPos = function(element) {"
	...
	/**
	 * Brings the element into the middle of the viewport (vertically), if the
	 * element is obstructed then the element will be brought into the middle of the
	 * viewport. This method requires JQuery. If JQuery is not loaded, this method
	 * will inject JQuery into the page and wait up to 10 seconds until loaded.
	 *
	 * @param element
	 *            the {@link WebElement} to bring into view.
	 */
	public static void bringIntoView(WebElement element) {
		...
		if (isClickObstructed(point, element)) {
			...
			jsexec.executeScript("function scrollIntoView(el) {" + "var offsetTop = $(el).offset().top;"
	...
	/**
	 * @param point
	 *            the {@link Point} of the {@link WebElement} to click
	 * @param element
	 *            the {@link WebElement}
	...
	private static boolean isClickObstructed(Point point, WebElement element) {
		...
		WebElement elementAtClick = (WebElement) executor(driver)
				.executeScript("return document.elementFromPoint(" + clickX + ", " + clickY + ");");
